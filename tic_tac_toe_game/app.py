...
# Coded by HtinLinAung(LeoKrypto-林浩）
# Date: 15.10.2024
...
class TicTacToeGame:
    ...
    def is_valid_move(self, move):
        """Check if a move is valid, Return True and otherwise False"""
    ...
    def has_winner(self):
        """Check if the game has a winner, Return True and otherwise False"""
        return self._has_winner

    def is_tied(self):
        """Check if the game is tied, Return True and otherwise False"""
        no_winner = not self._has_winner
        played_moves = (
            move.label for row in self._current_moves
            for move in row
        )
...
class TicTacToeBoard(tk.Tk):
    ...
    def play(self, event):
        ...
        move = Move(row, col, label=self._game.current_player.label)
    ...
    def reset_board(self):
        ...
        for btn in self._cells.keys():
            btn.config(highlightbackground="lightblue")
            btn.config(text="")
            btn.config(fg="black")
            #btn.state(["!disabled"])
